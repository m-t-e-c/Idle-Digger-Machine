...
namespace FIMSpace.AnimationTools
{
    ...
    public class ADClipSettings_IK : IADSettings
    {
        ...
        public class IKSet : INameAndIndex
        {
                ...
                if (IKType == EIKType.FootIK && LegMode == ELegMode.Grounding)
                {
                    ...
                    if (LegGrounding == ELegGroundingView.Analyze)
                    {
                        ...
                        if (FootDataAnalyze == null || FootDataAnalyze.analyzed == false)
                        {
                            ...
                            if (GUILayout.Button(new GUIContent("  Analyze animation clip using settings above", FGUI_Resources.Tex_Refresh))) { GenerateFootAnalyzeData(save, limb, clipMain.settingsForClip, ikSets, HeelGroundedTreshold, ToesGroundedTreshold, HoldOnGround, clipMain.ResetRootPosition); }
                        ...
                        else
                        {
                            ...
                            if (changed)
                            {
                                ...
                                if (GUILayout.Button(new GUIContent("  Re-Analyze animation clip using settings above", FGUI_Resources.Tex_Refresh))) { GenerateFootAnalyzeData(save, limb, clipMain.settingsForClip, ikSets, HeelGroundedTreshold, ToesGroundedTreshold, HoldOnGround, clipMain.ResetRootPosition); }
                                GUI.backgroundColor = preBC;
            ...
            private void GenerateFootAnalyzeData(AnimationDesignerSave save, ADArmatureLimb limb, AnimationClip clip, ADClipSettings_IK ikSets, float footTreshold, float toesTreshold, float holdOnGround, bool removeRootMotion)
            {
                GetClipFootAnalyze(save, limb, clip, footTreshold, toesTreshold, holdOnGround, removeRootMotion);
                ...
                if (syncGrLegIKSettingsToOtherLegs)
                {
                    for (int i = 0; i < save.Limbs.Count; i++)
                    {
                        ...
                        if (set == null) continue;
                        if (set.IsLegGroundingMode == false) continue;
                        set.GetClipFootAnalyze(save, save.Limbs[i], clip, footTreshold, toesTreshold, holdOnGround, removeRootMotion);
                    }
                }
            ...
            private void GetClipFootAnalyze(AnimationDesignerSave save, ADArmatureLimb limb, AnimationClip clip, float heelTreshold, float toesTreshold, float holdOnGround, bool removeRootMotion)
            {
